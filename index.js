...
/******LOAD OF MENU INPUT******/
const { charope } = require('./src/charope')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=12542123926:+51 924 318593\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '!'
blocked = []
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `tchau seu merda seca @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ pera ae mano calma o furico ⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					admin: '[❗]se voce for membro comum chama algum adm  ❌',
